...
class Book {
    ...
    constructor(metadata, sections) {
      ...
      { id, title: `EpubPress - ${date}` },
      Book.DEFAULT_METADATA
    );
        Object.keys(metadata || {}).forEach((metaKey) => {
            if (metadata[metaKey]) {
                self._metadata[metaKey] = Book.replaceCharacters(
          sanitizeHtml(metadata[metaKey])
        );
            }
        });

        this._ebook = nodepub.document(
      this._metadata,
      Book.DEFAULT_COVER_PATH,
      this.getToc
    );
