    ...
    static extractSectionContent(section) {
        ...
        return new Promise((resolve) => {
            ...
            readability(section.html, (error, article) => {
                updatedSection.title = article.title || Url.parse(section.url).host;
                updatedSection.title = updatedSection.title.replace('&', '&amp;');
                if (article && article.content) {
                    updatedSection.content = `<h1>${updatedSection.title}</h1>${article.content}`;
                } else {
