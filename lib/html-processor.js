  ...
  /*
    ...
    */
    ...
    static _filterDownloadedImages($, imgStatuses) {
        ...
        $('img').each((index, elem) => {
            const result = imgStatuses.find(
        status => status.src === $(elem).attr('src')
      );
        ...
        // Small images don't need to be huge...
                if (
          result.contentLength < SMALL_IMAGE_SIZE &&
          $(elem).css('width') === '100%'
        ) {
                    $(elem).css('width', '');
                }
    ...
    static extractImages(rootUrl, html) {
        ...
        .then((imgStatuses) => {
            const downloadedImages = HtmlProcessor._filterDownloadedImages(
            $,
            imgStatuses
          );
            resolve({
    ...
    */
    ...
    static removeHidden(selector, html) {
        return HtmlProcessor._proccessElements(selector, html, ($elem) => {
            ...
            if (
        /display:\s*none/.test(style) ||
        /(\w|^)visibility:\s*hidden/.test(style)
      ) {
                $elem.remove();
            }
        });
