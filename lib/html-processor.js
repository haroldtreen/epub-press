...
class HtmlProcessor {
    ...
    */
    ...
    static extractImages(rootUrl, html) {
        ...
        const imgMap = sources.reduce((map, src) => {
    ...
    static downloadImages(imgMap) {
        const requestPromises = Object.keys(imgMap).map((imgSrc) => {
            ...
            const image = new ContentDownloader(url, {
                path: Config.IMAGES_TMP,
                maxSize: MAX_IMAGE_SIZE,
            });

            return image.download().then((result) => {
                const newResult = Object.assign({ src: imgSrc }, result);
                return Promise.resolve(newResult);
            }).catch(error => Promise.resolve({ error, src: imgSrc }));
        });
