...
class HtmlProcessor {
    ...
    */
    ...
    static setFiletype(downloadInfo) {
        ...
        if (!path.extname(newPath)) {
            newPath += (HtmlProcessor.FILE_TYPES[downloadInfo.contentType] || '');
        }
        return newPath;
    }
            ...
            return new Promise((resolve) => {
                const req = request({
                    ...
                    if (error) {
                        ...
                        !HtmlProcessor.FILE_TYPES[downloadInfo.contentType] &&
                        !path.extname(downloadInfo.localPath)
                    ) {
                        downloadInfo = HtmlProcessor._setError(
                            downloadInfo,
    ...
    */
    ...
    static runHtmlOperations(html, operations) {
        let $ = typeof html === 'string' ? cheerio.load(html, { decodeEntities: false }) : html;
    ...
    static _proccessElements(selector, html, method) {
        const $ = typeof html === 'string' ? cheerio.load(html, { decodeEntities: false }) : html;
    ...
    static insertMissingParagraphTags(selector, html) {
        return HtmlProcessor._proccessElements(selector, html, ($elem) => {
            ...
            $elem.contents().each((index, elem) => {
                if (elem.type === 'text' && elem.data.length > 7) {
                    elemHtml = elemHtml.replace(elem.data, '<p>$&</p>');
                }
            });
            $elem.html(elemHtml);
        });
