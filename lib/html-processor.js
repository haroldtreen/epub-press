...
class HtmlProcessor {
        ...
        const requestPromises = Object.keys(imgMap).map((imgSrc) => {
            ...
            return new Promise((resolve) => {
                const req = request({
                    ...
                    if (error) {
                        log.exception('HtmlProcessor.downloadImage')(error);
                        ...
                        if (!path.extname(downloadInfo.localPath)) {
                            downloadInfo.success = false;
                            downloadInfo.error = 'No extension found';
                            log.warn('No extension', { imageInfo: downloadInfo });
                            resolve(downloadInfo);
                        } else {
    ...
    */
    ...
    static maximizeSize(selector, html) {
        return HtmlProcessor._proccessElements(selector, html, ($elem) => {
            const existingWidth = $elem.attr('width');
            const isPercentage = existingWidth && existingWidth.indexOf('%') > -1;

            if (existingWidth && isPercentage || Number(existingWidth) > 110) {
                $elem.attr('height', '');
                $elem.attr('width', '100%');
            } else if (!existingWidth) {
                $elem.attr('height', '');
                $elem.attr('width', '100%');
            }
        });
